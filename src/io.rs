...
impl Data {
    pub fn new(
        ...
        for i in 0..n_chromosomes {
            for &pos in &positions_per_chromosome[i] {
                let n_alleles = (exponential.sample(&mut rng) as f64).round().max(2.0) as usize;
                loci.push(Locus{
                    chromosome_id: i,
                    position: pos,
                    alleles: ["A", "T", "C", "G", "D"][0..n_alleles].iter().map(|&x| x.to_owned()).collect::<Vec<String>>(),
                    length: 1,
                    col_idx: (locus_allele_counter..(locus_allele_counter+(n_alleles as u64))).collect::<Vec<u64>>(),
                });
