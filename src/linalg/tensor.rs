...
impl GpuTensor {
    pub fn from_f32(
        ...
        ensure!(data.len() == shape.iter().product::<u32>() as usize, "The shape and data are incompatible!");
    ...
    pub fn to_vec_f32(
        ...
        let temp_buffer = ctx.device.create_buffer(
            &wgpu::BufferDescriptor {
                label: Some("tensor-readback"),
                size,
                usage: wgpu::BufferUsages::COPY_DST
                    | wgpu::BufferUsages::MAP_READ, // critical to be MAP_READ and not COPY_DST as in `from_f32`
...
mod tests {
    ...
    fn creates_1d_tensor() -> Result<()> {
        ...
        assert_eq!(tensor.buffer.size(), (4 * std::mem::size_of::<f32>()) as u64);
        Ok(())
    }
    #[test]
    fn creates_2d_tensor() -> Result<()> {
        ...
        assert_eq!(tensor.buffer.size(), (6 * std::mem::size_of::<f32>()) as u64);
        Ok(())
    }
    #[test]
    fn creates_3d_tensor() -> Result<()> {
        ...
        assert_eq!(tensor.buffer.size(),(24 * std::mem::size_of::<f32>()) as u64);
        Ok(())
    }
    #[test]
